...
export async function createClaim(claim: Omit<Claim, 'id' | 'created_at'>): Promise<Claim | null> {
  try {
    const { rows } = await sql`
...
export async function createClaimNote(note: Omit<ClaimNote, 'id' | 'timestamp' | 'author_name'>): Promise<ClaimNote | null> {
  try {
    const { rows } = await sql`
...
export async function createAdmin(admin: Omit<AdminUser, 'id' | 'created_at'>): Promise<AdminUser | null> {
  try {
    const { rows } = await sql`
...
export async function initializeDatabase() {
  try {
    ...
    await sql`
      ...
      VALUES 
        (1, 'Jane Smith', 'jane@claimbridge.com', 'admin'),
        (2, 'John Doe', 'john@claimbridge.com', 'super-admin')
